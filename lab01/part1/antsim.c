...
int main(int argc, char **args)
{
	...
	for (run = 0; run < runs; run++) {
		...
		/* For Part 2.6, reset to the original environment here WITHOUT affecting pheromone levels. */
		...
		for (iteration = 0; iteration < ITERS; iteration++) {
			...
			/* Load modified environment here for Part 2.6 at an appropriate time
...
void ant_strategy(ant_t * ant, cell_t n[3][3], int *dx, int *dy, double *pher)
{
	...
	//pher[ant->food] = species.pher_drop; // Uncomment this line for part 2.5
	pher[0] = species.pher_drop; // Comment this line for part 2.5
	...
	for (i = 0; i < 3; i++) {
		for (j = 0; j < 3; j++) {
			...
			//pher_total += pow(n[i][j].pher_qty[!ant->food], species.wght); // Uncomment this line for part 2.5
			pher_total += pow(n[i][j].pher_qty[0], species.wght); // Comment this line for part 2.5
		}
	}
	...
	if (pher_total == 0.0 || rnd() < species.wander) {
		...
		for (i = 0; i < 3; i++) {
			for (j = 0; j < 3; j++) {
				...
				//current_pher += pow(n[i][j].pher_qty[!ant->food], species.wght); // Uncomment this line for part 2.5
				current_pher += pow(n[i][j].pher_qty[0], species.wght); // Comment this line for part 2.5
